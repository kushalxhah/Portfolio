...
try {
  const diffOutput = execSync('git log -p -n 5 src/components/ProjectCard.css', { encoding: 'utf8' });
  fs.writeFileSync('git-diff.txt', diffOutput);
} catch (e) {
  fs.writeFileSync('git-diff.txt', 'Error: ' + e.message);
}
